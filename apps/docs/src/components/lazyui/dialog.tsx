          ...
          >
            {(title || showCloseButton) && (
              <div className="flex items-center justify-between px-6 pt-5 pb-1">
                {title && (
                  <motion.h3 className="font-medium text-lg" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}>
                    {title}
                  </motion.h3>
                )}
                ...
                {showCloseButton && (
                  <motion.button
                    className={`${variant.iconColor} p-2 rounded-full hover:bg-black/5 active:bg-black/10 transition-all ml-auto cursor-pointer`}
                    onClick={onClose}
            ...
            <div className="px-6 py-4">
              {description && (
                <motion.p className="text-base opacity-90 mb-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.15 }}>
                  {description}
                </motion.p>
              )}
            ...
            {(onConfirm || onClose) && (
              <motion.div className="px-6 py-4 bg-gray-50 flex justify-end gap-3" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}>
                {onClose && (
                  <motion.button
                    className={`cursor-pointer px-4 py-2 rounded-lg font-medium text-sm ${variant.secondaryButton} transition-all`}
                    onClick={onClose}
                ...
                {onConfirm && (
                  <motion.button
                    className={`cursor-pointer px-4 py-2 rounded-lg font-medium text-sm ${variant.primaryButton} transition-all`}
                    onClick={onConfirm}
