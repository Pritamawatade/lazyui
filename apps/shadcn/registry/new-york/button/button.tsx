...
export function Button({ variant = "primary", size = "medium", children = "Click Me", icon, onClick, disabled = false, className = "", ...props }: ButtonProps) {
  ...
  const handleClick = (e: MouseEvent<HTMLButtonElement>) => {
    ...
    if (onClick) onClick(); // safer check
  };
  ...
  return (
    <motion.button
      ...
      {...props}
    >
      <motion.div className="relative z-10 flex items-center justify-center gap-2" animate={{ y: isHovered ? -2 : 0 }} transition={{ duration: 0.2 }}>
        {icon && (
          <motion.span
